local function parse_line(linenr)
	...
	if not line or not ok or not query then
		return nil
	end
...
function M.text()
	...
	table.insert(result, {
		(" + %d %s "):format(vim.v.foldend - vim.v.foldstart, require("config.ui.icons").misc.l),
		"FoldedText",
	})
...
local function set_fold_hl()
	vim.api.nvim_set_hl(0, "FoldedText", {
		link = "TSComment",
	})
end
...
vim.api.nvim_create_autocmd("ColorScheme", {
	...
	group = require("config.util").augroup("FoldText"),
})

_G.Fold = M
vim.opt.foldtext = [[v:lua.Fold.text()]]
