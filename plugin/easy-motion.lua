...
local function easy_motion()
	local needle = vim.fn.nr2char(vim.fn.getchar() --[[@as number]])
	...
	for lines_i, line_text in ipairs(lines) do
		...
		if vim.fn.foldclosed(line_idx) == -1 then
			for i = 1, #line_text do
				if line_text:sub(i, i) == needle and char_idx <= #EM_CHARS then
					local overlay_char = EM_CHARS[char_idx]
					local linenr = line_idx_start + lines_i - 2
					local col = i - 1
					local id = vim.api.nvim_buf_set_extmark(bufnr, EASYMOTION_NS, linenr, col + 1, {
